...
/-- Query Sage for the boundary data of `∂_n` on the complex given by `ffcExpr`.
Takes `Expr`s for the `FiniteFacetComplex` value and the dimension `n`; returns
the decoded `(n, domain basis, codomain basis, matrix)` tuple. Used by the
`#boundary`, `#boundary_check`, and `#boundary_goal` commands — not what the
`boundary_verify` tactic wants (that one already has the data in the goal). -/
...
/-- Compile-and-run `verifyBoundaryDataB F k dom cod d` inside `MetaM`, returning
the resulting `Bool`. Answers "is this boundary data correct?" in one shot;
...
/-- Break the boundary check into its three component Bools:
`(domain-basis OK, codomain-basis OK, shape-and-entries OK)`. Useful on the
failure path to localize *what kind* of problem the data has. -/
...
/-- Run `firstMismatch dom cod d` and return the first entry where the given
matrix disagrees with the boundary spec: `(i, j, actual, expected, τ, σ)`.
Returns `none` if no entry mismatch is found — which typically means the
problem is elsewhere (wrong shape, bad basis). -/
